...
def Seq2Seq(output_dim, output_length, batch_input_shape=None,
    ...
    encoder = RecurrentSequential(readout=True, state_sync=inner_broadcast_state,
                                  ...
                                  return_states=broadcast_state, name='recurrentseq')
    for _ in range(depth[0]):
        encoder.add(LSTMCell(hidden_dim, batch_input_shape=(shape[0], hidden_dim)))
        encoder.add(Dropout(dropout))

    dense1 = TimeDistributed(Dense(hidden_dim, name='dense1'))
    dense1.supports_masking = True
    dense2 = Dense(output_dim, name='dense2')
    ...
    _input = Input(batch_shape=shape, name='input')
    ...
    if broadcast_state:
        ...
        states = None
    encoded_seq2 = dense2(encoded_seq)
    inputs = [_input]
    if teacher_force:
