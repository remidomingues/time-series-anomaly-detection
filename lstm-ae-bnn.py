...
class LSTMAEBNN:
    ...
    def _encode_sequence(self, X, fit=False):
        if fit:
            self.one_hot_encoder = {x: i for x, i in enumerate(set(X))}
            self.one_hot_decoder = {i: x for i, x in self.one_hot_encoder.items()}
        ids = np.array([self.one_hot_encoder[x] for x in X])
        out = np.zeros((X.shape[0], len(self.one_hot_encoder)), dtype=int)
        for i, idx in enumerate(ids):
            out[i][idx] = 1
        return out

    def _decode_sequence(self, X):
        """
        X (n_actions, one-hot encoding (can be continuous))
        """
        indices = [np.argmax(x) for x in X]
        return np.array([self.one_hot_decoder[_id] for _id in indices])

    def fit(self, X_train, epochs=10, hidden_dim=5, hidden_layers=1, verbose=False): #bnn_hdim=10, bnn_hlayer=1, bnn_epochs=1000):
        """
        X_train: (n_actions,) action IDs
        """
        # Split the training data into overlapping batches, based on a sliding window
        X_train = self._encode_sequence(X_train, fit=True)
    ...
    def predict_seq(self, X_test): #, metric='distance'):
        ...
        """
        encoded_X_test = self._encode_sequence(X_test)
        batches = self._sliding_batches(encoded_X_test, self.sequence_length, self.window_overlap)
        out = self.model.predict(batches)  # (N sequences, M events, D unique actions (one-hot encoding))
